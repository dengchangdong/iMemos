...
export const markdownRenderer = {
  ...
  renderToHtml(text) {
    ...
    html = html.replace(CONFIG.REGEX.MD_CODE_BLOCK, (match, lang, code) => 
      utils.createHtml`<pre data-language="${lang || 'plaintext'}" class="relative bg-gray-100 dark:bg-slate-800 rounded-[6px] my-4 p-4 overflow-auto"><code class="whitespace-pre language-${lang || 'plaintext'}">${utils.escapeHtml(code)}</code></pre>`
    );
