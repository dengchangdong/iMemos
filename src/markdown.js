...
export const createMarkdownRenderer = () => {
  // 使用优化的缓存系统
  const cache = utils.createCache(CONFIG.CACHE.MAX_SIZE, CONFIG.DEFAULTS.CACHE_TTL);
  ...
  return {
    ...
    renderToHtml(text) {
      ...
      // 代码块（保留原始缩进）
      html = html.replace(CONFIG.REGEX.MD.CODE_BLOCK, (match, lang, code) => 
      ...
      // 行内代码
      html = html.replace(CONFIG.REGEX.MD.INLINE_CODE, '<code class="bg-gray-100 dark:bg-gray-800 px-1.5 py-0.5 rounded text-sm">$1</code>');
      
      // 标题 
      html = html.replace(CONFIG.REGEX.MD.H1, '<h1 class="text-2xl font-bold mt-6 mb-4">$1</h1>');
      html = html.replace(CONFIG.REGEX.MD.H2, '<h2 class="text-xl font-bold mt-5 mb-3">$1</h2>');
      html = html.replace(CONFIG.REGEX.MD.H3, '<h3 class="text-lg font-bold mt-4 mb-2">$1</h3>');
      
      // 引用
      html = html.replace(CONFIG.REGEX.MD.QUOTE, '<blockquote class="pl-4 border-l-4 border-gray-300 dark:border-gray-600 my-4 text-gray-600 dark:text-gray-400">$1</blockquote>');
      
      // 列表项
      html = html.replace(CONFIG.REGEX.MD.LIST_ITEM, '<li class="ml-4 list-disc">$1</li>');
      html = html.replace(CONFIG.REGEX.MD.NUM_LIST, '<li class="ml-4 list-decimal">$2</li>');
      ...
      // 格式化文本
      html = html.replace(CONFIG.REGEX.MD.BOLD, '<strong>$1</strong>');
      html = html.replace(CONFIG.REGEX.MD.ITALIC, '<em>$1</em>');
      
      // 处理图片 - 添加懒加载和预览支持
      html = html.replace(CONFIG.REGEX.MD.IMAGE, 
        '<img src="$2" alt="$1" class="rounded-lg max-w-full my-4" loading="lazy" data-preview="true" />'
      );
      
      // 处理链接 - 排除微信链接（由特殊链接处理器处理）
      html = html.replace(CONFIG.REGEX.MD.LINK, (match, text, url) => {
        // 保持URL原样，不对特殊字符进行转义
    ...
    processSpecialLinks(html) {
      ...
      // 处理GitHub仓库
      html = processLink(CONFIG.REGEX.SOCIAL.GITHUB, (match, repo) => {
...
// 简化外部调用接口
export const simpleMarkdown = (text) => markdownRenderer.render(text);
