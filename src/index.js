...
function parseSpecialLinks(content) {
  ...
  // 微信公众号文章 - 简化版本，使用链接文本作为标题
  replaceWithPlaceholder(LINK_PATTERNS.wechat, (match) => {
    // 提取URL的最后部分作为标题
    let title = "微信公众号文章";
    // 从正文中尝试查找链接对应的文本
    const linkTextMatch = content.match(new RegExp(`([^<>]*?)${match.replace(/[.*+?^${}()|[\]\\]/g, '\\$&')}([^<>]*?)`, 'i'));
    if (linkTextMatch) {
      // 提取链接前后的文本，看是否有更好的描述
      const beforeText = linkTextMatch[1]?.trim();
      const afterText = linkTextMatch[2]?.trim();
      
      // 如果链接前有文本且看起来像标题，使用它
      if (beforeText && beforeText.length > 0 && beforeText.length < 100 && !beforeText.endsWith('://')) {
        title = beforeText;
      } 
      // 否则尝试使用链接后的文本
      else if (afterText && afterText.length > 0 && afterText.length < 100) {
        title = afterText;
      }
    }
